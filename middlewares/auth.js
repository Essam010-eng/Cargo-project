...
const apperrore = require("../helpers/globalerrorehandler");
...
const register = async (req, res, next) => {
    try {
        ...
        if (existingUser) {
            return next(new apperrore(400, "User already exists"));
        }
        if (role === "admin") {
            const adminExists = await User.findOne({ role: "admin" });
            if (adminExists) {
                
                return next(new apperrore(400, "An admin already exists in the system"));
            }
        }
